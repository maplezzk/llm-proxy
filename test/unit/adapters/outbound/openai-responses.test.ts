...
const req: CanonicalRequest = {
  ...
  system: 'be concise',
  generation: { stream: false, maxTokens: 200 },
  reasoning: { source: 'client', effort: 'medium', summary: 'concise' },
  tools: [
    { kind: 'computer', name: 'computer', schema: {}, displayWidth: 800 },
    { kind: 'function', name: 'click', namespace: 'mcp__ui__', schema: { type: 'object' } },
  ],
};
describe('unit/adapters/outbound/openai-responses', () => {
  it('仅投影已解析的 reasoning effort 与 summary', () => {
  ...
  it.skip('namespace 工具名称展平（当前出站重复追加 __）', () => {
    ...
    expect(body.tools).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ name: 'mcp__ui__click' }),
      ]),
    );
  });
});
