// P1.12 阶段 B：从 legacy-test/adapter/handlers.test.ts 机械迁移（node:test → vitest）
// 断言语义保持不变，仅替换测试栈与断言 API。
// 说明：node:test 的 describe(name, { timeout }, fn) 选项形式 vitest 不支持，
// 已去掉该选项；vitest 全局 testTimeout=90s（vitest.config.ts）远大于原 10s，行为等价。
...
describe('adapter/handlers', () => {
  ...
  it('通过适配器端点转发请求成功', async () => {
    ...
    expect((data.choices[0].message.content as string).includes('ok')).toBeTruthy()
  })
  ...
  it('请求体缺少 model 字段返回 400', async () => {
    ...
    expect(resp.status).toBe(400)
    const data = await resp.json()
    expect((data.error.message as string).includes('model')).toBeTruthy()
  })
