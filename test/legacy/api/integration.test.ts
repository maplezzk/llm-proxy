// P1.12 阶段 B：从 legacy-test/api/integration.test.ts 机械迁移（node:test → vitest）
// 断言语义保持不变，仅替换测试栈与断言 API。
// 说明：node:test 的 describe(name, { timeout }, fn) 选项形式 vitest 不支持，已去掉；
// vitest 全局 testTimeout=90s（vitest.config.ts）远大于原 15s，行为等价。
...
describe('integration', () => {
  ...
  it('OpenAI 格式 → 非流式 → 同协议转发', async () => {
    ...
    expect((msg.content as string).includes('gpt-4o-test'), 'model 应替换为上游模型名').toBeTruthy()
  })
  ...
  it('OpenAI 格式 → 流式 → 同协议', async () => {
    ...
    expect(text.includes('content'), '流式应含 content').toBeTruthy()
    expect(text.includes('[DONE]'), '流式应以 [DONE] 结束').toBeTruthy()
  })
  ...
  it('Anthropic 格式 → 非流式 → 跨协议响应转换', async () => {
    ...
    expect((data.content as Array<Record<string, unknown>>)[0].type).toBe('text')
  })
  ...
  it('Anthropic 格式 → 流式 → 跨协议 SSE 转换', async () => {
    ...
    const text = await resp.text()
    expect(text.includes('event: content_block_start'), '应含 content_block_start').toBeTruthy()
    expect(text.includes('event: message_stop'), '应含 message_stop').toBeTruthy()
    expect(text.includes('text_delta'), '应含 text_delta').toBeTruthy()
  })
  ...
  it('Admin UI: GET /admin 返回 200 + text/html', async () => {
    ...
    expect(contentType.startsWith('text/html'), `期望 text/html，实际 ${contentType}`).toBeTruthy()
    const html = await resp.text()
    // 稳定行为：有构建产物 → 真实 admin-ui.html；无构建产物 → '<h1>Admin UI not found</h1>'。
    // 两者均含开标签与闭标签，所以断言 HTML 结构而非具体内容。
    expect(/<[a-zA-Z][^>]*>/.test(html), '响应体应含 HTML 开标签').toBeTruthy()
    expect(/<\/[a-zA-Z][^>]*>/.test(html), '响应体应含 HTML 闭标签').toBeTruthy()
  })
  ...
  it('管理 API: GET /api/admin/config 返回原始 Key', async () => {
    ...
    const data = await resp.json()
    expect(data.success).toBe(true)
    expect(data.data.providers[0].api_key).toBe('sk-test')
  })
  ...
  it('不存在的 model name 返回 404', async () => {
    ...
    expect(resp.status).toBe(404)
    const data = await resp.json()
    expect((data.error?.message as string).includes('未找到')).toBeTruthy()
  })
  ...
  it('适配器端点同协议转发成功', async () => {
    ...
    const data = await resp.json()
    expect(data.object).toBe('chat.completion')
    expect((data.choices[0].message.content as string).includes('gpt-4o-test')).toBeTruthy()
  })
  ...
  it('管理 API: GET /api/admin/adapters 返回适配器列表', async () => {
    ...
    const data = await resp.json()
    expect(data.success).toBe(true)
    expect(data.data.adapters.length > 0).toBeTruthy()
    expect(data.data.adapters[0].name).toBe('my-tool')
    expect(data.data.adapters[0].type).toBe('openai')
    expect(data.data.adapters[0].models[0].sourceModelId).toBe('gpt-test')
    expect(data.data.adapters[0].models[0].targetModelId).toBe('gpt-4o-test')
    expect(data.data.adapters[0].models[0].status).toBe('ok')
  })
  ...
  it('POST /v1/responses → 非流式 → 同协议返回 Responses 格式', async () => {
    ...
    expect((msg.content[0].text as string).includes('gpt-4o-resp')).toBeTruthy()
    // 验证上游收到了 /v1/responses 请求
    const req = mockRequests[mockRequests.length - 1]
    expect(req.url).toBe('/v1/responses')
    expect(req.body.model).toBe('gpt-4o-resp')
    expect(req.body.input).toBe('hi')
    expect(req.body.instructions).toBe('Be helpful.')
  })
  ...
  it('POST /v1/responses → 流式 → 同协议 SSE 穿透', async () => {
    ...
    const text = await resp.text()
    expect(text.includes('event: response.created'), '应有 response.created').toBeTruthy()
    expect(text.includes('event: response.output_text.delta'), '应有 output_text.delta').toBeTruthy()
    expect(text.includes('event: response.completed'), '应有 response.completed').toBeTruthy()
    expect(text.includes('gpt-4o-resp'), '内容应含模型名').toBeTruthy()
    const req = mockRequests[mockRequests.length - 1]
    expect(req.url).toBe('/v1/responses')
  })
  ...
  it('POST /v1/responses → 跨协议 → Anthropic 格式返回', async () => {
    ...
    expect(Array.isArray(req.body.messages)).toBeTruthy()
  })
  ...
  it('适配器端点 /my-tool/v1/responses 转发成功', async () => {
    ...
    const data = await resp.json()
    expect(data.object).toBe('response')
    expect((data.output[0].content[0].text as string).includes('gpt-4o-resp')).toBeTruthy()
    const req = mockRequests[mockRequests.length - 1]
    expect(req.url).toBe('/v1/responses')
    expect(req.body.model).toBe('gpt-4o-resp')
  })
  ...
  it('Mock 上游收到正确的 Responses/Chat/Messages 请求', () => {
    ...
    expect(resReqs.length > 0, '/v1/responses 应有请求').toBeTruthy()
    const msgReqs = mockRequests.filter((r) => r.url === '/v1/messages')
    expect(msgReqs.length > 0, '跨协议应有 /v1/messages 请求').toBeTruthy()
    const chatReqs = mockRequests.filter((r) => r.url === '/v1/chat/completions')
    expect(chatReqs.length > 0, '/v1/chat/completions 应有请求').toBeTruthy()
  })
})
