...
function responseContent(blocks: CanonicalBlock[]): unknown {
  return blocks.map((block) => {
    ...
    if (block.kind === 'image')
      return block.source.kind === 'url'
        ? { type: 'input_image', image_url: block.source.url, detail: block.source.detail }
        : block.source.kind === 'file_id'
          ? { type: 'input_image', file_id: block.source.fileId }
          : {
              type: 'input_image',
              image_url: `data:${block.source.mediaType};base64,${block.source.data}`,
            };
    if (block.kind === 'thinking' || block.kind === 'reasoning')
      return { type: 'reasoning', summary: [{ type: 'summary_text', text: block.text }] };
  ...
  for (const tool of tools) {
    if (tool.kind === 'computer') {
      result.push({
        ...
        display_number: tool.displayNumber,
      });
      continue;
    }
...
export const openAiResponsesOutbound: OutboundAdapter = {
  ...
  encode(request: CanonicalRequest): WireBody {
      ...
      input: request.messages
        ...
        .flatMap((m) => {
          const item: Record<string, unknown> = {
            ...
            content: responseContent(m.blocks),
          };
          return [item];
        }),
    };
    if (typeof request.system === 'string') body.instructions = request.system;
      ...
      body.instructions = request.system
        .map((block) => (block.kind === 'text' ? block.text : '[image]'))
        .join('');
    ...
    const toolList = responseTools(request.tools);
