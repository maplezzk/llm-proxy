...
function content(blocks: CanonicalBlock[]): unknown {
  ...
  for (const block of blocks) {
    ...
    else if (block.kind === 'thinking') parts.push({type:'thinking', thinking:block.text, ...(block.signature ? {signature:block.signature} : {})});
    else if (block.kind === 'image') { const source=block.source; parts.push(source.kind==='url' ? {type:'image_url',image_url:{url:source.url,detail:source.detail}} : source.kind==='base64' ? {type:'image_url',image_url:{url:`data:${source.mediaType};base64,${source.data}`}} : {type:'text',text:`[image:file_id=${source.fileId}]`}); }
    else if (block.kind === 'tool_use') parts.push({type:'tool_use',id:block.id,name:block.namespace ? `${block.namespace}__${block.name}` : block.name,input:block.input});
    else if (block.kind === 'tool_result') parts.push({type:'tool_result',tool_use_id:block.toolUseId,content:typeof block.content==='string' ? block.content : content(block.content),...(block.isError ? {is_error:true} : {})});
...
function message(message: CanonicalMessage): Record<string, unknown> {
  ...
  if (message.role==='assistant') {
    const calls=message.blocks.filter((block): block is Extract<CanonicalBlock,{kind:'tool_use'}>=>block.kind==='tool_use');
    const thinking=message.blocks.filter((block): block is Extract<CanonicalBlock,{kind:'thinking'}>=>block.kind==='thinking').map(block=>block.text).join('');
    if (thinking) result.reasoning_content=thinking;
    if (calls.length) result.tool_calls=calls.map(call=>({id:call.id,type:'function',function:{name:call.namespace ? `${call.namespace}__${call.name}` : call.name,arguments:JSON.stringify(call.input)}}));
  }
  return result;
}
function openAiTools(tools: CanonicalTool[]|undefined): unknown[]|undefined {
  ...
  for (const tool of tools) {
    ...
    output.push({type:'function',function:{name:tool.namespace ? `${tool.namespace}__${tool.name}` : tool.name,description:tool.description,parameters:tool.schema}});
  }
