...
export async function* decodeOpenAIChatStream(
  ...
  for await (const frame of readSse(stream)) {
    ...
    if (usage) latestUsage = mergeUsage(latestUsage, usage);
    ...
    if (!messageStarted && (delta.role === 'assistant' || deltaHasContent)) yield* ensureMessageStart();

    const reasoning = toText(delta.reasoning_content ?? delta.reasoning);
    if (reasoning !== '') {
      yield* ensureMessageStart();
      if (!openBlocks.has('chat:thinking')) {
